...
namespace FSPortalNET.Sitios.Transeste.code
{
	/// <summary>
		...
		private string SaveFile()
		{
			if (FSPortal.Variables.App.Page.Request.Files.Count > 0) {
				...
				if (filePosted != null && filePosted.ContentLength > 0) {
					...
					UpdateQueryBuilder sqB = new UpdateQueryBuilder();
					if (camion) {
						sqB.TableSource = Variables.App.prefijoTablas + "BDGT.CAMIONESCONTROLESISO";
					}
					if (remolque) {
						sqB.TableSource = Variables.App.prefijoTablas + "BDGT.REMOLQUESCONTROLESISO";
					}
					sqB.Assignments.AddAssignment("FECHAVENCIMIENTO", fechaIso);
					sqB.Assignments.AddAssignment("OBSERVACIONES", descripcion);
					AndWhere andW = new AndWhere();
					if (camion) {
						andW.Add(new SimpleWhere(sqB.TableSource, "CODIGOCAMION", Comparison.Equals, id));
					}
					if (remolque) {
						andW.Add(new SimpleWhere(sqB.TableSource, "CODIGOREMOLQUE", Comparison.Equals, id));
					}
					andW.Add(new SimpleWhere(sqB.TableSource, "CODIGOTIPOCONTROLISO", Comparison.Equals, id));
					sqB.Where = andW;
					
					FSQueryBuilder.Constants.Dbms.dbmsType = DBMSType.Oracle;
		            		
					if (db.ExecuteNonQuery(sqB.BuildQuery()) == 0) {
