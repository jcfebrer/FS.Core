...
CKEDITOR.dialog.add('link', function(a) {
    var b = function() {
        ...
        j = /^(_(?:self|top|parent|blank))$/,
        k = /^javascript:void\(location\.href='mailto:'\+String\.fromCharCode\(([^)]+)\)(?:\+'(.*)')?\)$/,
        l = /^javascript:([^(]+)\(([^)]+)\)$/,
        ...
        o = function(B, C) {
            ...
            if (E = D.match(d))
                if (x == 'encode')D = D.replace(k, function(Y, Z, aa) { return 'mailto:' + String.fromCharCode.apply(String, Z.split(',')) + (aa && v(aa)); });
                else if (x)
                    D.replace(l, function(Y, Z, aa) {
                        if (Z == y.name) {
                            I.type = 'email';
            ...
            for (W = 0; W < V.length; W++) {
                X = V[W];
                V[W] = { name: X.getAttribute('name'), id: X.getAttribute('id') };
            }
            this._.selectedElement = C;
            return I;
        },
        p = function(B, C) { if (C[B])this.setValue(C[B][this.id] || ''); },
        q = function(B) { return p.call(this, 'target', B); },
        r = function(B) { return p.call(this, 'adv', B); },
        s = function(B, C) {
            if (!C[B])C[B] = {};
            C[B][this.id] = this.getValue() || '';
        },
        t = function(B) { return s.call(this, 'target', B); },
        u = function(B) { return s.call(this, 'adv', B); };

    function v(B) { return B.replace(/\\'/g, "'"); };

    function w(B) { return B.replace(/'/g, '\\$&'); };
    ...
    if (x && x != 'encode') {
        ...
        x.replace(/^([^(]+)\(([^)]+)\)$/, function(B, C, D) {
            y.name = C;
            y.params = [];
            D.replace(/[^,\s]+/g, function(E) { y.params.push(E); });
        });
    }
    ...
    return{
        ...
        contents: [
            {
                ...
                elements: [
                    { id: 'linkType', type: 'select', label: a.lang.link.type, 'default': 'url', items: [[a.lang.link.toUrl, 'url'], [a.lang.link.toAnchor, 'anchor'], [a.lang.link.toEmail, 'email']], onChange: c, setup: function(B) { if (B.type)this.setValue(B.type); }, commit: function(B) { B.type = this.getValue(); } }, {
                        type: 'vbox',
                        id: 'urlOptions',
                        children: [
                            {
                                ...
                                children: [
                                    {
                                        ...
                                        type: 'text', id: 'url', label: a.lang.common.url, required: true, onLoad: function() { this.allowOnChange = true; },
                                        onKeyUp: function() {
                                            var G = this;
                                ...
                                setup: function(B) { if (!this.getDialog().getContentElement('info', 'linkType'))this.getElement().show(); }
                        ...
                        children: [
                            {
                                type: 'fieldset', id: 'selectAnchorText', label: a.lang.link.selectAnchor,
                                setup: function(B) {
                                    if (B.anchors.length > 0)this.getElement().show();
                                    else this.getElement().hide();
                                },
                                children: [
                                    {
                                        type: 'hbox', id: 'selectAnchor',
                                        children: [
                                            {
                                                type: 'select', id: 'anchorName', 'default': '', label: a.lang.link.anchorName, style: 'width: 100%;', items: [['']],
                                                setup: function(B) {
                                                    var E = this;
                                                    E.clear();
                                                    E.add('*');
                                                ...
                                                type: 'select', id: 'anchorId', 'default': '', label: a.lang.link.anchorId, style: 'width: 100%;', items: [['']],
                                                setup: function(B) {
                                                    var D = this;
                                                    D.clear();
                                                    D.add('*');
                        ...
                        setup: function(B) { if (!this.getDialog().getContentElement('info', 'linkType'))this.getElement().hide(); }
                    }, {
                        type: 'vbox', id: 'emailOptions', padding: 1,
                        children: [
                            {
                                type: 'text', id: 'emailAddress', label: a.lang.link.emailAddress, required: true,
                                validate: function() {
                                    var B = this.getDialog();
                                ...
                                type: 'text', id: 'emailSubject', label: a.lang.link.emailSubject, setup: function(B) { if (B.email)this.setValue(B.email.subject); },
                                commit: function(B) {
                                    if (!B.email)B.email = {};
                                    B.email.subject = this.getValue();
                                }
                            }, {
                                type: 'textarea', id: 'emailBody', label: a.lang.link.emailBody, rows: 3, 'default': '', setup: function(B) { if (B.email)this.setValue(B.email.body); },
                                commit: function(B) {
                                    if (!B.email)B.email = {};
                ...
                id: 'target', label: a.lang.link.target, title: a.lang.link.target,
                elements: [
                    {
                        type: 'hbox', widths: ['50%', '50%'],
                        children: [
                            {
                                type: 'select', id: 'linkTargetType', label: a.lang.common.target, 'default': 'notSet', style: 'width : 100%;', items: [[a.lang.common.notSet, 'notSet'], [a.lang.link.targetFrame, 'frame'], [a.lang.link.targetPopup, 'popup'], [a.lang.common.targetNew, '_blank'], [a.lang.common.targetTop, '_top'], [a.lang.common.targetSelf, '_self'], [a.lang.common.targetParent, '_parent']], onChange: b, setup: function(B) { if (B.target)this.setValue(B.target.type); },
                                commit: function(B) {
                                    if (!B.target)B.target = {};
                                    B.target.type = this.getValue();
                                }
                            }, {
                                type: 'text', id: 'linkTargetName', label: a.lang.link.targetFrameName, 'default': '', setup: function(B) { if (B.target)this.setValue(B.target.name); },
                                commit: function(B) {
                                    if (!B.target)B.target = {};
        ...
        onOk: function() {
            var B = { href: 'javascript:void(0)/*' + CKEDITOR.tools.getNextNumber() + '*/' }, C = [], D = { href: B.href }, E = this, F = this.getParentEditor();
            ...
            if (D.target)
                if (D.target.type == 'popup') {
                    var R = ["window.open(this.href, '", D.target.name || '', "', '"], S = ['resizable', 'status', 'location', 'toolbar', 'menubar', 'fullscreen', 'scrollbars', 'dependent'], T = S.length, U = function(ag) { if (D.target[ag])S.push(ag + '=' + D.target[ag]); };
                    ...
                    if (D.target.type != 'notSet' && D.target.name)B.target = D.target.name;
                    else C.push('target');
            ...
            if (!this._.selectedElement) {
                ...
                var aa = new CKEDITOR.style({ element: 'a', attributes: B });
                ...
                if (ac.getAttribute('name'))ac.addClass('cke_anchor');
                else ac.removeClass('cke_anchor');
