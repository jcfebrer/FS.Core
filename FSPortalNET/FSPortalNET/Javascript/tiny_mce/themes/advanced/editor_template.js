    ...
    e.create("tinymce.themes.AdvancedTheme", {
        ...
        _importClasses: function(k) {
            ...
            if (j.getLength() == 0) {
                f(i.dom.getClasses(), function(n, l) {
                    var m = "style_" + l;
                    i.formatter.register(m, { inline: "span", classes: n["class"], selector: "*" });
                    j.add(n["class"], m);
                });
            }
        },
        _createStyleSelect: function(m) {
            ...
            l = j.createListBox("styleselect", {
                ...
                onselect: function(n) {
                    i.execCommand("mceToggleFormat", false, n);
                    return false;
                }
            });
            i.onInit.add(function() {
                ...
                if (n) {
                    ...
                    f(i.getParam("theme_advanced_styles", "", "hash"), function(r, q) {
                        ...
                        if (r) {
                            p = "style_" + (o++);
                            i.formatter.register(p, { inline: "span", classes: r });
        ...
        _createFontSelect: function() {
            ...
            if (k) {
                f(i.getParam("theme_advanced_fonts", j.settings.theme_advanced_fonts, "hash"), function(m, l) { k.add(i.translate(l), m, { style: m.indexOf("dings") == -1 ? "font-family:" + m : "" }); });
            }
            return k;
        },
        ...
        _createBlockFormats: function() {
            ...
            if (k) {
                f(j.editor.getParam("theme_advanced_blockformats", j.settings.theme_advanced_blockformats, "hash"), function(m, l) { k.add(j.editor.translate(l != m ? l : i[m]), m, { "class": "mce_formatPreview mce_" + m }); });
            }
            return k;
        },
        _createForeColorMenu: function() {
            ...
            if (k.theme_advanced_more_colors) {
                l.more_colors_func = function() { j._mceColorPicker(0, { color: m.value, func: function(n) { m.setColor(n); } }); };
            }
        ...
        renderUI: function(k) {
            ...
            if (!r.getParam("accessibility_focus")) {
                b.add(d.add(j, "a", { href: "#" }, "<!-- IE -->"), "focus", function() { tinyMCE.get(r.id).focus(); });
            }
        ...
        resizeTo: function(i, l) {
            var j = this.editor, k = j.settings, m = d.get(j.id + "_tbl"), n = d.get(j.id + "_ifr");
        ...
        _addStatusBar: function(m, j) {
            ...
            if (w.theme_advanced_resizing) {
                ...
                p.onPostRender.add(function() {
                    b.add(p.id + "_resize", "mousedown", function(A) {
                        var r, o, D, z, x, y, C, n, B, s;
                        ...
                        z = A.screenX;
                        x = A.screenY;
                        s = d.get(v.editor.id + "_ifr");
                        y = n = s.clientWidth;
                        C = B = s.clientHeight;
                        r = b.add(d.doc, "mousemove", t);
                        o = b.add(p.getDoc(), "mousemove", t);
                        D = b.add(d.doc, "mouseup", function(E) {
                            b.remove(d.doc, "mousemove", r);
                            b.remove(p.getDoc(), "mousemove", o);
                            b.remove(d.doc, "mouseup", D);
                            if (w.theme_advanced_resizing_use_cookie) {
                                a.setHash("TinyMCE_" + p.id + "_size", { cw: n, ch: B });
                            }
                        });
                    });
                });
        ...
        _nodeChanged: function(o, x, l, u, j) {
            var A = this, i, w = 0, z, q, B = A.settings, y, k, r;
            e.each(A.stateControls, function(n) { x.setActive(n, o.queryCommandState(A.controls[n][1])); });

            function m(p) {
                var s, n = j.parents, t = p;
                if (typeof(p) == "string") {
                    t = function(v) { return v.nodeName == p; };
                }
            ...
            x.setActive("visualaid", o.hasVisual);
            x.setDisabled("undo", !o.undoManager.hasUndo() && !o.typing);
            x.setDisabled("redo", !o.undoManager.hasRedo());
            x.setDisabled("outdent", !o.queryCommandState("Outdent"));
            i = m("A");
            if (q = x.get("link")) {
                if (!i || !i.name) {
                    q.setDisabled(!i && u);
            ...
            i = m("IMG");
            if (q = x.get("image")) {
                q.setActive(!!i && l.className.indexOf("mceItem") == -1);
            }
            if (q = x.get("styleselect")) {
                A._importClasses();
                q.select(function(n) { return !!o.formatter.match(n); });
            }
            if (q = x.get("formatselect")) {
                i = m(d.isBlock);
                if (i) {
                    q.select(i.nodeName.toLowerCase());
                }
            }
            m(function(p) {
                if (p.nodeName === "SPAN") {
            ...
            if (q = x.get("fontselect")) {
                q.select(function(n) { return n.replace(/^([^,]+).*/, "$1").toLowerCase() == r; });
            }
            if (q = x.get("fontsizeselect")) {
                if (B.theme_advanced_runtime_fontsize && !k && !y) {
                    k = o.dom.getStyle(l, "fontSize", true);
                }
                q.select(function(n) {
                    if (n.fontSize && n.fontSize === k) {
                        return true;
                    }
                    if (n["class"] && n["class"] === y) {
                        return true;
                    }
                });
            }
            if (B.theme_advanced_path && B.theme_advanced_statusbar_location) {
                i = d.get(o.id + "_path") || d.add(o.id + "_path_row", "span", { id: o.id + "_path" });
                d.setHTML(i, "");
                m(function(C) {
                    var p = C.nodeName.toLowerCase(), s, v, t = "";
                    if (C.nodeType != 1 || C.nodeName === "BR" || (d.hasClass(C, "mceItemHidden") || d.hasClass(C, "mceItemRemoved"))) {
                        return;
                    }
                    if (z = d.getAttrib(C, "mce_name")) {
                        p = z;
                    }
                    if (e.isIE && C.scopeName !== "HTML") {
                        p = C.scopeName + ":" + p;
                    }
                    p = p.replace(/mce\:/g, "");
                    switch (p) {
                    ...
                    case"i":
                        ...
                        break;
                    case"img":
                        if (z = d.getAttrib(C, "src")) {
                            t += "src: " + z + " ";
                        }
                        break;
                    case"a":
                        if (z = d.getAttrib(C, "name")) {
                            t += "name: " + z + " ";
                            p += "#" + z;
                        }
                        if (z = d.getAttrib(C, "href")) {
                            t += "href: " + z + " ";
                        }
                        break;
                    case"font":
                        if (z = d.getAttrib(C, "face")) {
                            t += "font: " + z + " ";
                        }
                        if (z = d.getAttrib(C, "size")) {
                            t += "size: " + z + " ";
                        }
                        if (z = d.getAttrib(C, "color")) {
                            t += "color: " + z + " ";
                        }
                        break;
                    case"span":
                        if (z = d.getAttrib(C, "style")) {
                            t += "style: " + z + " ";
                        }
                        break;
                    }
                    if (z = d.getAttrib(C, "id")) {
                        t += "id: " + z + " ";
                    }
                    if (z = C.className) {
                        z = z.replace(/\b\s*(webkit|mce|Apple-)\w+\s*\b/g, "");
                        if (z) {
                            t += "class: " + z + " ";
                            if (d.isBlock(C) || p == "img" || p == "span") {
                                p += "." + z;
                            }
                        }
                    }
                    p = p.replace(/(html:)/g, "");
                    p = { name: p, node: C, title: t };
                    A.onResolveName.dispatch(A, p);
                    t = p.title;
                    p = p.name;
                    v = d.create("a", { href: "javascript:;", onmousedown: "return false;", title: t, "class": "mcePath_" + (w++) }, p);
                    if (i.hasChildNodes()) {
