(function() {
    var c = tinymce.each, d = null, a = { paste_auto_cleanup_on_paste: true, paste_block_drop: false, paste_retain_style_properties: "none", paste_strip_class_attributes: "mso", paste_remove_spans: false, paste_remove_styles: false, paste_remove_styles_if_webkit: true, paste_convert_middot_lists: true, paste_convert_headers_to_strong: false, paste_dialog_width: "450", paste_dialog_height: "400", paste_text_use_dialog: false, paste_text_sticky: false, paste_text_notifyalways: false, paste_text_linebreaktype: "p", paste_text_replacements: [[/\u2026/g, "..."], [/[\x93\x94\u201c\u201d]/g, '"'], [/[\x60\x91\x92\u2018\u2019]/g, "'"]] };

    function b(e, f) { return e.getParam(f, a[f]); }
    ...
    tinymce.create("tinymce.plugins.PastePlugin", {
        init: function(e, f) {
            ...
            g.onPreProcess.add(function(j, k) { e.execCallback("paste_preprocess", j, k); });
            g.onPostProcess.add(function(j, k) { e.execCallback("paste_postprocess", j, k); });
            ...
            e.addCommand("mceInsertClipboardContent", function(j, k) { i(k, true); });
            if (!b(e, "paste_text_use_dialog")) {
                e.addCommand("mcePasteText", function(k, j) {
                    ...
                    if ((e.pasteAsPlainText) && (!l.get("tinymcePasteText"))) {
                        if (b(e, "paste_text_sticky")) {
                            e.windowManager.alert("Paste is now in plain text mode. Click again to toggle back to regular paste mode. After you paste something you will be returned to regular paste mode.");
                        } else {
                            e.windowManager.alert("Paste is now in plain text mode. Click again to toggle back to regular paste mode.");
                        }
            ...
            function h(s) {
                ...
                m = p.add(r, "div", { id: "_mcePaste", "class": "mcePaste" }, "\uFEFF");
                if (r != e.getDoc().body) {
                ...
                if (tinymce.isIE) {
                    ...
                    window.setTimeout(function() {
                        ...
                        c(n, function(u) {
                            c(p.select("div.mcePaste", u), function(v) { p.remove(v, 1); });
                            t += (p.select("> span.Apple-style-span div", u)[0] || p.select("> span.Apple-style-span", u)[0] || u).innerHTML;
                        });
                        c(n, function(u) { p.remove(u); });
                        if (q) {
                            l.setRng(q);
                        }
                        i({ content: t });
            ...
            if (b(e, "paste_auto_cleanup_on_paste")) {
                if (tinymce.isOpera || /Firefox\/2/.test(navigator.userAgent)) {
                    ...
                    e.onPaste.addToTop(function(j, k) { return h(k); });
                }
            }
        ...
        _preProcess: function(i, f) {
            ...
            if (/class="?Mso|style="[^"]*\bmso-|w:WordDocument/i.test(k) || f.wordContent) {
                ...
                if (b(l, "paste_retain_style_properties").replace(/^none$/i, "").length == 0) {
                    ...
                    e([
                        [/<span\s+style\s*=\s*"\s*mso-spacerun\s*:\s*yes\s*;?\s*"\s*>([\s\u00a0]*)<\/span>/gi, function(o, h) { return(h.length > 0) ? h.replace(/./, " ").slice(Math.floor(h.length / 2)).split("").join("\u00a0") : ""; }], [
                            /(<[a-z][^>]*)\sstyle="([^"]*)"/gi, function(u, h, t) {
                                var v = [], o = 0, r = p(g(t).replace(/&quot;/gi, "'"), ";");
                                c(r, function(s) {
                                    var w, y, z = p(s, ":");

                                    function x(A) { return A + ((A !== "0") && (/\d$/.test(A))) ? "px" : ""; }
            ...
            if (j !== "none") {
                function n(r, o) {
                    ...
                    var h = q(p(o.replace(/^(["'])(.*)\1$/, "$2"), " "), function(s) { return(/^(?!mso)/i.test(s)); });
        ...
        _postProcess: function(h, j) {
            ...
            if (b(f, "paste_remove_styles") || (b(f, "paste_remove_styles_if_webkit") && tinymce.isWebKit)) {
                ...
                if (tinymce.isWebKit) {
                    c(i.select("*", j.node), function(k) { k.removeAttribute("_mce_style"); });
                }
            }
        },
        ...
        _insertBlockContent: function(l, h, m) {
            ...
            if (f && !/TD|TH/.test(f.nodeName)) {
                j = h.split(f, j);
                c(h.create("div", 0, m).childNodes, function(r) { q = j.parentNode.insertBefore(r.cloneNode(true), j); });
                p(q);
            } else {
                h.setOuterHTML(j, m);
        ...
        _legacySupport: function() {
            ...
            e.addCommand("mcePasteWord", function() { e.windowManager.open({ file: f.url + "/pasteword.htm", width: parseInt(b(e, "paste_dialog_width")), height: parseInt(b(e, "paste_dialog_height")), inline: 1 }); });
            if (b(e, "paste_text_use_dialog")) {
                e.addCommand("mcePasteText", function() { e.windowManager.open({ file: f.url + "/pastetext.htm", width: parseInt(b(e, "paste_dialog_width")), height: parseInt(b(e, "paste_dialog_height")), inline: 1 }); });
            }
