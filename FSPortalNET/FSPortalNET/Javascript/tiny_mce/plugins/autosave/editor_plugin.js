    ...
    e.create("tinymce.plugins.AutoSave", {
        init: function(i, j) {
            ...
            i.addButton(g, {
                ...
                onclick: function() {
                    if (i.getContent().replace(/\s|&nbsp;|<\/?p[^>]*>|<br[^>]*>/gi, "").length > 0) {
        ...
        getInfo: function() { return{ longname: "Auto save", author: "Moxiecode Systems AB", authorurl: "http://tinymce.moxiecode.com", infourl: "http://wiki.moxiecode.com/index.php/TinyMCE:Plugins/autosave", version: e.majorVersion + "." + e.minorVersion }; },
        getExpDate: function() { return new Date(new Date().getTime() + this.editor.settings.autosave_retention).toUTCString(); },
                ...
                function() {
                    ...
                    if (e.isIE) {
                        ...
                        return{
                            ...
                            removeItem: function(l) { i.getElement().removeAttribute(l); }
                        };
                    }
                },
            ], function(l) {
        ...
        restoreDraft: function() {
            var h = this, i = h.storage;
            if (i) {
                content = i.getItem(h.key);
                if (content) {
                    h.editor.setContent(content);
                    h.onRestoreDraft.dispatch(h, { content: content });
                }
            }
        },
        hasDraft: function() {
