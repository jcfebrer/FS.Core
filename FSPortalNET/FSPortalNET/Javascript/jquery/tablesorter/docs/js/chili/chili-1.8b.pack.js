(function($) {
    ...
    $.metaobjects = function(options) {
        options = $.extend({ context: document, clean: true, selector: 'object.metaobject' }, options);
        ...
        return $(options.selector, options.context).each(function() {
            ...
            $('> param[@name=metaparam]', this).each(function() { $.extend(settings, jsValue(this.value)); });
    ...
    $.fn.chili = function(options) {
        ...
        function cook(ingredients, recipe) {
            function prepareStep(stepName, step) {
                ...
                steps.push({ stepName: stepName, exp: "(" + exp + ")", length: 1 + (exp.replace(/\\./g, "%").replace(/\[.*?\]/g, "%").match(/\((?!\?)/g) || []).length, replacement: (step.replacement) ? step.replacement : book.defaultReplacement });
            }
            ...
            function knowHow() {
                ...
                for (var i = 0; i < steps.length; i++) {
                    var exp = steps[i].exp;
                    exp = exp.replace(/\\\\|\\(\d+)/g, function(m, aNum) { return!aNum ? m : "\\" + (prevLength + 1 + parseInt(aNum, 10)); });
                    exps.push(exp);
        ...
        function makeDish(el, recipePath) {
            ...
            if (ChiliBook.preFixCopy) {
                $(el).parents().filter("pre").bind("mousedown", function() { ChiliBook.preElement = this; }).bind("mouseup", function() {
        ...
        function getPath(recipeName, options) {
            var settingsDef = { recipeFolder: book.recipeFolder, recipeFile: recipeName + ".js", stylesheetFolder: book.stylesheetFolder, stylesheetFile: recipeName + ".css" };
            var settings;
    ...
    $(function() {
        ...
        if (ChiliBook.preFixCopy) {
            ...
            }).bind("mousedown", function() { ChiliBook.preContent = ""; }).bind("mouseup", function() { ChiliBook.preElement = null; });
        }
    });
})(jQuery);