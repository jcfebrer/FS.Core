...
strSQL = strSQL & "WHERE " & portal.variablesForum.strDbTable & "PMMessage.UsuarioID=" & portal.variablesForum.lngLoggedInUserID & " "
...
If rsPmMessage.EOF AND intRecordPositionPageNum > 1 Then response.redirect("pm_inbox.aspx?PN=1"
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 <tr><form name="frmDelete" method="post" action="pm_delete_message.aspx?Page=<% = intRecordPositionPageNum %>" OnSubmit="return confirm('<% = portal.variablesForum.strTxtDeletePrivateMessageAlert %>')">
 <td>
<table width="<% = portal.variablesForum.strTableVariableWidth %>" border="0" cellspacing="0" cellpadding="4" align="center">
 <tr> 
  <td width="60%"><span class="lgText"><img src="<% = portal.variablesForum.strImagePath %>subject_folder.gif" alt="<% = portal.variablesForum.strTxtSubjectFolder %>" align="middle"> <% = portal.variablesForum.strTxtPrivateMessenger & ": " & portal.variablesForum.strTxtInbox %></span></td>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="<% = portal.variablesForum.strTableBgColour %>">
   ...
   <table width="100%" border="0" cellspacing="1" cellpadding="3" height="14" bgcolor="<% = portal.variablesForum.strTableBgColour %>">
    <tr>
     ...
     <td bgcolor="<% = portal.variablesForum.strTableTitleColour %>" width="22%" height="2" class="tHeading" background="<% = portal.variablesForum.strTableTitleBgImage %>"><% = portal.variablesForum.strTxtMessageFrom %></td>
     <td bgcolor="<% = portal.variablesForum.strTableTitleColour %>" width="31%" height="2" class="tHeading" background="<% = portal.variablesForum.strTableTitleBgImage %>"><% = portal.variablesForum.strTxtDate %></td>
         <td bgcolor="<% = portal.variablesForum.strTableTitleColour %>" width="5%" align="center" height="2" class="tHeading" background="<% = portal.variablesForum.strTableTitleBgImage %>"><%
...
If rsPmMessage.EOF Then
	...
	Response.Write vbCrLf & "<td bgcolor=""" & portal.variablesForum.strTableColour & """ background=""" & portal.variablesForum.strTableBgImage & """ colspan=""5"" class=""text"">" & portal.variablesForum.strTxtNoPrivateMessages & " " & portal.variablesForum.strTxtInbox & "<input type=""hidden"" name=""chkDelete"" value=""-1""></td>"
...
Else 
    ...
    <tr> 
     ...
     <td bgcolor="<% If (intRecordLoopCounter MOD 2 = 0 ) Then Response.Write(portal.variablesForum.strTableEvenRowColour) Else Response.Write(portal.variablesForum.strTableOddRowColour) %>" background="<% = portal.variablesForum.strTableBgImage %>" width="39%" class="text"><% Response.Write("<a href=""pm_show_message.aspx?ID=" & rsPmMessage("PM_ID") & """ target=""_self"">" & rsPmMessage("PM_Tittle") & "</a>") %></td>
     <td bgcolor="<% If (intRecordLoopCounter MOD 2 = 0 ) Then Response.Write(portal.variablesForum.strTableEvenRowColour) Else Response.Write(portal.variablesForum.strTableOddRowColour) %>" background="<% = portal.variablesForum.strTableBgImage %>" width="22%" class="text"><a href="JavaScript:openWin('pop_up_profile.aspx?PF=<% = rsPmMessage("From_ID") %>','profile','toolbar=0,location=0,status=0,menubar=0,scrollbars=1,resizable=1,width=590,height=425')"><% = rsPmMessage("usuario") %></a></td>
     ...
     <td bgcolor="<% If (intRecordLoopCounter MOD 2 = 0 ) Then Response.Write(portal.variablesForum.strTableEvenRowColour) Else Response.Write(portal.variablesForum.strTableOddRowColour) %>" background="<% = portal.variablesForum.strTableBgImage %>" width="5%" class="text" align="center"><input type='checkbox' name="chkDelete" value="<% = rsPmMessage("PM_ID") %>"></td>
    </tr><%
...
%></td>
    ...
    <table width="<% = portal.variablesForum.strTableVariableWidth %>" border="0" cellspacing="0" cellpadding="2" align="center">
     <tr>
      <td class="text" colspan="2"><% Response.Write(strLoggedInusuario & ", " & portal.variablesForum.strTxtYouHave & " " & intNumOfPMs & " " & portal.variablesForum.strTxtPrivateMessagesYouCanReceiveAnother & " " & (intNumPrivateMessages-intNumOfPMs) & " " & portal.variablesForum.strTxtOutOf & " " & intNumPrivateMessages) %></td>
     </tr>
     <tr> 
      <td class="text"><table width="300" border="0" cellspacing="0" cellpadding="1" bgcolor="<% = portal.variablesForum.strTableBorderColour %>">
        <tr> 
         <td> <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<% = portal.variablesForum.strTableColour %>" background="<% = portal.variablesForum.strTableBgImage %>">
           <tr><td colspan="3" align="center" class="smText"><% = portal.variablesForum.strTxtYourInboxIs & " " & FormatPercent((intNumOfPMs / intNumPrivateMessages), 0) & " " & portal.variablesForum.strTxtFull %></td></tr>
           <tr><td colspan="3"><img src="<% = portal.variablesForum.strImagePath %>bar_graph_image.gif" width="<% = FormatPercent((intNumOfPMs / intNumPrivateMessages), 0) %>" height="11"></td></tr>
           <tr>
...
If intTotalNumOfPages > 1 Then   
	...
	If intRecordPositionPageNum > 1 Then Response.Write("<a href=""pm_inbox.aspx?PN=" & intRecordPositionPageNum - 1 & """>&lt;&amp;lt&nbsp;" & portal.variablesForum.strTxtPrevious & "</a>&nbsp;")
	...
	For intPageLoopCounter = 1 to intTotalNumOfPages  
		...
		Response.Write (vbCrLf & "		  <option value=""pm_inbox.aspx?PN=" & intPageLoopCounter & """")
	...
	If intRecordPositionPageNum <> intTotalNumOfPages Then Response.Write("&nbsp;<a href=""pm_inbox.aspx?PN=" & intRecordPositionPageNum + 1 & """>" & portal.variablesForum.strTxtNext & "&nbsp;&gt;&gt;</a>")
...
If Request.QueryString("MSG") = "DEL" Then
		Response.Write("<script  language=""JavaScript"">")
		Response.Write("alert('" & portal.variablesForum.strTxtMeassageDeleted & ".');")
