...
' Salt can be used to help protect claves.  When a clave is first stored in a
' database generate a salt value also.  Concatenate the salt value with the clave, 
' and then encrypt it using the HashEncode function below.  Store both the salt value,
' and the encrypted value in the database.  When a clave needs to be verified, take 
' the clave concatenate the salt from the database.  Encode it using the HashEncode 
' function below.  If the result matches the the encrypted clave stored in the
' database, then it is a match.  If not then the clave is invalid.
'
'
' Note: claves become case sensitive when using this encryption.
' For more information on clave HASH Encoding, and SALT visit: http://local.15seconds.com/issue/000217.htm
'
...
' Free to distribute as long as code is not modified, and header is kept intact
'
' The Usuarios makes no warranties as to the validity, and/or authenticity of this code.
