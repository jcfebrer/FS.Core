...
namespace FSCompressCore
{
    ...
    public class Zip : IDisposable
    {
        ...
        public static Zip Create(string _filename, string _comment)
        {
            Stream stream = new FileStream(_filename, FileMode.Create, FileAccess.ReadWrite);

            Zip zip = Create(stream, _comment);
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static Zip Create(Stream _stream, string _comment)
        {
            Zip zip = new Zip();
            zip.Comment = _comment;
        ...
        public void AddFile(Compression _method, string _pathname, string _filenameInZip, string _comment)
        {
            ...
            AddStream(_method, _filenameInZip, stream, System.IO.File.GetLastWriteTime(_pathname), _comment);
            stream.Close();
        }
        /// <summary>
        ...
        /// <param name="_comment">Comment for stored file</param>
        public void AddStream(Compression _method, string _filenameInZip, Stream _source, System.DateTime _modTime, string _comment)
        {
            if (Access == FileAccess.Read)
                throw new InvalidOperationException("Writing is not alowed");
        ...
        public bool ExtractFile(ZipFileEntry _zfe, string _filename)
        {
            ...
            System.IO.File.SetCreationTime(_filename, _zfe.ModifyTime);
            System.IO.File.SetLastWriteTime(_filename, _zfe.ModifyTime);
        ...
        public bool ExtractFile(ZipFileEntry _zfe, Stream _stream)
        {
            ...
            Int64 bytesPending = _zfe.FileSize;
        ...
        public static bool RemoveEntries(ref Zip _zip, List<ZipFileEntry> _zfes)
        {
            ...
            try
            {
                Zip tempZip = Zip.Create(tempZipName, string.Empty);
                ...
                System.IO.File.Delete(_zip.FileName);
                System.IO.File.Move(tempZipName, _zip.FileName);

                _zip = Zip.Open(_zip.FileName, _zip.Access);
            }
            catch
            {
                return false;
            ...
            {
                if (System.IO.File.Exists(tempZipName))
                    System.IO.File.Delete(tempZipName);
                if (System.IO.File.Exists(tempEntryName))
                    System.IO.File.Delete(tempEntryName);
            }
            return true;
        }
        #endregion
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            do
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        _zfe.Crc32 = Zip.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                    }
                }
        ...
        */
		private uint DateTimeToDosTime(System.DateTime _dt)
        {
            return (uint)(
                (_dt.Second / 2) | (_dt.Minute << 5) | (_dt.Hour << 11) |
                (_dt.Day << 16) | (_dt.Month << 21) | ((_dt.Year - 1980) << 25));
        }
		private System.DateTime DosTimeToDateTime(uint _dt)
        {
