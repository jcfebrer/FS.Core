...
{
    public partial class DBWebBrowser
    {
        #region Delegates
                ...
                if (CheckIsChildValidPage)
                {
                    ...
                    if (BlackList != null)
                    {
                        ...
                        if (BlackList.Contains(url))
                        {
                            e.Cancel = true;
                            WebBrowser1.Stop();
                            MessageBox.Show("[BLACKLIST] La página seleccionada no es válida para niños.", "No válida",
                                            MessageBoxButtons.OK);
                        }
                    }
                    ...
                    if (!IsValidChildPage(e.Url.DnsSafeHost))
                    {
                        e.Cancel = true;
                        WebBrowser1.Stop();
                        MessageBox.Show("[DNS] La página seleccionada no es válida para niños.", "No válida",
                                        MessageBoxButtons.OK);
                    }
                }
        ...
        public bool Navigate(string urlToLoad)
        {
            try
            {
                ...
                {
                    MessageBox.Show("La página seleccionada no es válida para niños", "No válida", MessageBoxButtons.OK);
                }
            }
            catch (Exception ex)
            {
        ...
        private void CheckBadWord()
        {
            if (WebBrowser1.Document != null)
                if (WebBrowser1.Document.Body != null)
                {
                    string docText = WiworBrowser.Controls.DBFunctions.RemoveExpressionSignals(WiworBrowser.Controls.DBFunctions.RemoveHtmlTags(WebBrowser1.DocumentText));
                    ...
                    foreach (string badWord in BadWords)
                    {
                        if (docText.Contains(" " + badWord + " "))
                        {
                            ...
                            MessageBox.Show(string.Format("Página con contenido no adecuado para menores. Palabra: [{0}]", badWord));
                            break;
                        }
                    }
                }
                ...
                foreach (HtmlElement he in WebBrowser1.Document.GetElementsByTagName("a"))
                {
                    ...
                    if (imgs.Count > 0)
                    {
                        ...
                        if (src != "")
                        {
                            ...
                            {
                                if (WiworBrowser.Controls.DBFunctions.IsUri(src))
                                {
                                    Uri url = new Uri(src);
        ...
        {
            WiworBrowser.Controls.DBImageProcess skin = new WiworBrowser.Controls.DBImageProcess();
            Bitmap bm;
        ...
        private void RemoveIFramesScript2()
        {
            try
            {
                ...
                IHTMLDocument2 doc = WiworBrowser.Controls.CrossFrameIE.GetDocumentFromBrowser(WebBrowser1);
                for (int fo = 0; fo < doc.frames.length; fo++)
                {
                    object objF = fo;
                    IHTMLWindow2 obj = (IHTMLWindow2) doc.frames.item(ref objF);
                    IHTMLDocument2 obj2 = WiworBrowser.Controls.CrossFrameIE.GetDocumentFromWindow(obj);
