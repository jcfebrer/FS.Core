...
namespace WiworBrowser.Controls
{
    ...
    /// </summary>
    public class DBZip : IDisposable
    {
        /// <summary>
        ...
        public static DBZip Create(string _filename, string _comment)
        {
            Stream stream = new FileStream(_filename, FileMode.Create, FileAccess.ReadWrite);

            DBZip zip = Create(stream, _comment);
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static DBZip Create(Stream _stream, string _comment)
        {
            DBZip zip = new DBZip();
            zip.Comment = _comment;
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static DBZip Open(Stream _stream, FileAccess _access)
        {
        ...
        public static bool RemoveEntries(ref DBZip _zip, List<ZipFileEntry> _zfes)
        {
            ...
            try
            {
                DBZip tempZip = DBZip.Create(tempZipName, string.Empty);
                ...
                _zip = DBZip.Open(_zip.FileName, _zip.Access);
            }
            catch
            {
                return false;
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            do
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        _zfe.Crc32 = DBZip.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                    }
                }
