...
namespace FSAutomatizeWeb.DownloadManager
{
    public class Downloader
    {
        ...
        private void StartDownloadThreadProc(object objSegmentCount)
        {
            ...
            int segmentCount = Math.Min((int)objSegmentCount, Convert.ToInt32(ConfigurationManager.AppSettings["MaxSegments"]));
            ...
            do
            {
                ...
                {
                    lastError = ex;
                    if (currentTry < Convert.ToInt32(ConfigurationManager.AppSettings["MaxRetries"]))
                    {
                        SetState(DownloaderState.WaitingForReconnect);
                        Thread.Sleep(TimeSpan.FromSeconds(Convert.ToInt32(ConfigurationManager.AppSettings["RetryDelay"])));
                    }
                    else
                    {
                        SetState(DownloaderState.NeedToPrepare);
        ...
        private bool RestartFailedSegments()
        {
            ...
            for (int i = 0; i < this.Segments.Count; i++)
            {
                if (Segments[i].State == SegmentState.Error &&
                    ...
                    (Convert.ToInt32(ConfigurationManager.AppSettings["MaxRetries"]) == 0 ||
                    Segments[i].CurrentTry < Convert.ToInt32(ConfigurationManager.AppSettings["MaxRetries"])))
                {
                    hasErrors = true;
                    TimeSpan ts = DateTime.Now - Segments[i].LastErrorDateTime;

                    if (ts.TotalSeconds >= Convert.ToInt32(ConfigurationManager.AppSettings["RetryDelay"]))
                    {
                    ...
                    {
                        delay = Math.Max(delay, Convert.ToInt32(ConfigurationManager.AppSettings["RetryDelay"]) * 1000 - ts.TotalMilliseconds);
                    }
                }
            }
            ...
            lock (segments)
            {
                for (int i = 0; i < this.segments.Count; i++)
                {
                    ...
                    if (oldSegment.State == SegmentState.Downloading &&
                        oldSegment.Left.TotalSeconds > Convert.ToInt32(ConfigurationManager.AppSettings["MinSegmentLeftToStartNewSegment"]) &&
                        oldSegment.MissingTransfer / 2 >= Convert.ToInt32(ConfigurationManager.AppSettings["MinSegmentSize"]))
                    {
