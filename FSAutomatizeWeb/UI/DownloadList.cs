...
using FSAutomatizeWeb.DownloadManager;
using System.Collections;
using System.IO;
using System.Diagnostics;
using System.Configuration;

namespace FSAutomatizeWeb.UI
{
    public partial class DownloadList : UserControl
    {
        ...
        public DownloadList()
        {
            ...
            FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.DownloadAdded += new EventHandler<DownloaderEventArgs>(Instance_DownloadAdded);
            FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.DownloadRemoved += new EventHandler<DownloaderEventArgs>(Instance_DownloadRemoved);
            FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.DownloadEnded += new EventHandler<DownloaderEventArgs>(Instance_DownloadEnded);

            for (int i = 0; i < FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.Downloads.Count; i++)
            {
                AddDownload(FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.Downloads[i]);
            }
        ...
        public void NewFileDownload(string fileName, string url, string location, int segments)
        {
            ...
            if (rl.ProtocolProviderType != null)
            {
                ...
                if (segments > Convert.ToInt32(ConfigurationManager.AppSettings["MaxSegments"]))
                {
                    segments = Convert.ToInt32(ConfigurationManager.AppSettings["MaxSegments"]);
                }
                ...
                Downloader download = FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.Add(
                        rl,
                        null,
                        localFile,
                        segments,
        ...
        private string GetLocalFile(string title, string url, string folder)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(folder))
                    folder = ConfigurationManager.AppSettings["DownloadFolder"];
        ...
        {
            FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.PauseAll();
            UpdateList();
        }
                ...
                try
                {
                    ...
                    DownloadsAction(
                        delegate(Downloader d, ListViewItem item)
                        {
                            ...
                            FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.RemoveDownload(d);
                        }
                    );
                }
                finally
        ...
        {
            using (FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.LockDownloadList(false))
            {
                lvwDownloads.BeginUpdate();
                try
                {
        ...
        public void RemoveCompleted()
        {
            ...
            try
            {
                FSAutomatizeWeb.DownloadManager.DownloadManager.Instance.ClearEnded();
                UpdateList();
            }
            finally
            {
