...
namespace FSGraphicsCore
{
    ...
    public static unsafe class PixelGrabber
    {
        ...
        public static byte[] GrabPixels(int[] pixels, Rectangle rectangle, PixelFormat pf)
        {
            ...
            switch (pf)
            {
                case PixelFormat.Format32bppArgb | PixelFormat.Format32bppRgb | PixelFormat.Format32bppPArgb:
                    ...
                    break;
                case PixelFormat.Format16bppRgb565 | PixelFormat.Format16bppRgb555 | PixelFormat.Format16bppGrayScale |
        ...
        public static int[] GrabPixelsInt(Bitmap img)
        {
            return GrabPixelsInt(img, img.PixelFormat);
        }

        public static int[] GrabPixelsInt(Bitmap img, PixelFormat pf)
        {
            var array = new int[img.Width * img.Height];
            var bmp = img.LockBits(new Rectangle(0, 0, img.Width, img.Height), ImageLockMode.ReadWrite, pf);
            var PixelSize = 4;
        ...
        /// <param name="pf">The pixel format that should be used</param>
        /// <returns></returns>
        public static int[] GrabPixelsInt(Bitmap img, int x, int y, int w, int h, PixelFormat pf)
        {
        ...
        public static int[] CopyPixelsInt(int[] pixels, int scanline, int x, int y, int w, int h)
        {
            var size = w * h;
            ...
            var s = 0;
            var p = y * scanline + x;
            //LogUtil.TraceInfo("Data offset: " + p);
            for (var i = 0; i < size; i++, s++, p++)
            {
                ...
                if (p >= pixels.Length) p = pixels.Length - 1;
                ourPixels[i] = pixels[p];
            }

            //LogUtil.TraceInfo("Data end: " + --p);
            return ourPixels;
        }
        ...
        public static Bitmap GrabImage(byte[] data, int w, int h, PixelFormat pf)
        {
            ...
            switch (pf)
            {
                ...
                case PixelFormat.Format24bppRgb:
                    m = 3;
                    break;
                case PixelFormat.Format16bppRgb565 | PixelFormat.Format16bppRgb555 | PixelFormat.Format16bppArgb1555 |
        ...
        /// <returns>A bitmap containing the pixel data</returns>
        public static Bitmap GrabImageInt(int w, int h, int[] data)
        {
        ...
        public static Bitmap GetSubImage(Bitmap bmp, Rectangle rect)
        {
            try
            {
                var data = GrabPixelsInt(bmp, rect.X, rect.Y, rect.Width, rect.Height, bmp.PixelFormat);
                return GrabImageInt(rect.Width, rect.Height, data);
            }
            catch (Exception)
            {
                return bmp;
        ...
        {
            var data1 = GrabPixelsInt(paramBitmap1, paramRectangle.X, paramRectangle.Y, paramRectangle.Width,
                paramRectangle.Height, paramBitmap1.PixelFormat);
            var data2 = GrabPixelsInt(paramBitmap2, paramRectangle.X, paramRectangle.Y, paramRectangle.Width,
        ...
        public static Rectangle GetChangeArea(Bitmap bmp1, Bitmap bmp2, Rectangle rect)
        {
            ...
            var w = bmp1.Width;
            var h = bmp1.Height;
            var pixels1 = GrabPixelsInt(bmp1);
            var pixels2 = GrabPixelsInt(bmp2);
            ...
            //LogUtil.TraceInfo("Single pixel modified");
            //else
            //{
            //LogUtil.TraceInfo("Modified part (rectangle): " + minx1 + "," + miny1 + "<->" + maxx2 + "," + maxy2);
