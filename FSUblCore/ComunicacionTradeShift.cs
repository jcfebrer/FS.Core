...
namespace FSUblCore
{
	...
	{
        FSNetworkCore.OAuth OAuthBase = new FSNetworkCore.OAuth();
		...
		private HttpWebRequest CreateOAuthRequest(Uri url, Guid tradeshiftTenantId, string method)
		{
			...
			string sig = OAuthBase.GenerateSignature(url, consumerKey, consumerSecret, token, tokenSecret, method, timeStamp, nonce, FSNetworkCore.OAuth.SignatureTypes.HMACSHA1, out normalizedUrl,
