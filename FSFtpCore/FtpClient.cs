...
namespace FSFtpCore
{
    ...
    /// FSFtp gets in return. Please review the Debug example
    /// below for information on how to add TraceListeners for capturing
    /// the convorsation between FSFtp and the server.
    /// </summary>
    /// <example>The following example illustrates how to assist in debugging
    /// FSFtp by getting a transaction log from the server.
	...
	public class FtpClient : IDisposable {
		...
		public void DeleteDirectory(string path) {
            DeleteDirectory(path, false, 0, false);
        }
		...
		public void DeleteDirectory(string path, bool force) {
            DeleteDirectory(path, force, 0, false);
        }
		...
		/// <example><code source="..\Examples\DeleteDirectory.cs" lang="cs" /></example>
		public void DeleteDirectory(string path, bool force, bool fastMode) {
		...
		public void DeleteDirectory(string path, bool force, FtpListOption options) {
            DeleteDirectory(path, force, options, false);
        }
		...
		/// <param name="fastMode">An experimental fast mode that file listing is only requested for once. This improves bandwidth usage and response time.</param>
        /// <example><code source="..\Examples\DeleteDirectory.cs" lang="cs" /></example>
		public void DeleteDirectory(string path, bool force, FtpListOption options, bool fastMode) {
			FtpReply reply;
			string ftppath = path.GetFtpPath();
        ...
        delegate void AsyncDeleteDirectory(string path, bool force, FtpListOption options, bool fastMode);
		...
		public IAsyncResult BeginDeleteDirectory(string path, AsyncCallback callback, object state) {
            return BeginDeleteDirectory(path, true, 0, false, callback, state);
        }
		...
		/// <returns>IAsyncResult</returns>
        /// <example><code source="..\Examples\BeginDeleteDirectory.cs" lang="cs" /></example>
		public IAsyncResult BeginDeleteDirectory(string path, AsyncCallback callback, object state, bool fastMode) {
        ...
        {
            return BeginDeleteDirectory(path, force, 0, false, callback, state);
        }
		...
		/// <returns>IAsyncResult</returns>
        /// <example><code source="..\Examples\BeginDeleteDirectory.cs" lang="cs" /></example>
		public IAsyncResult BeginDeleteDirectory(string path, bool force, AsyncCallback callback, object state, bool fastMode) {
        ...
        public bool UploadFile(string file)
        {
            ...
            FtpDataType fileDataType = FtpDataType.Binary;
            if (file.ToLower().EndsWith(".txt")) fileDataType = FtpDataType.ASCII;

            using (Stream stream = OpenWrite(fileName, fileDataType))
            {
                try
                {
                ...
                {
                    FtpTrace.WriteLine("Error upload file: " + ex.ToString());
                    return false;
                }
                finally
                {
