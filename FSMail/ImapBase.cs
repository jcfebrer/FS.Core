...
namespace FSMail
{
	public class ImapBase
	{
		...
		/// Imap uid copy command
		/// </summary>
		protected const string IMAP_UIDCREATE_FOLDER_COMMAND = "CREATE";
		/// <summary>
        ...
        /// <summary>
        /// The imap message content transfer encoding
        /// </summary>
        protected const string IMAP_MESSAGE_CONTENT_TRANSFER_ENCODING = "content-transfer-encoding";
		/// <summary>
        ...
        /// <summary>
        /// The imap message quoted printable encoding
        /// </summary>
        protected const string IMAP_MESSAGE_QUOTED_PRINTABLE_ENCODING = "quoted-printable";
		/// <summary>
		...
		protected ImapResponseEnum Connect(string sHost, int nPort, bool sslEnabled = false)
		{
			...
			try
			{
				...
				if (sslEnabled)
				{
					...
					SslStream sslStrm = new SslStream(m_oImapServ.GetStream(), false, new RemoteCertificateValidationCallback(ValidateServerCertificate), null);
		...
		protected ImapResponseEnum SendAndReceive(string command, ref ArrayList sResultArray)
		{
			...
			ImapResponseEnum eImapResponse = ImapResponseEnum.IMAP_SUCCESS_RESPONSE;
			Log.TraceInfo(sCommand.TrimEnd(IMAP_COMMAND_EOL.ToCharArray()));
		...
		/// <returns></returns>
		protected string ReceiveBuffer(int nSize)
		{
			string sBuffer = "";
			int nRead = -1;
			int tSize = 0;
