...
namespace FSDns
{
    ...
    public class Resolver
    {
        ...
        {
            get { return m_DnsServers.ToArray(); }
            set
            {
                m_DnsServers.Clear();
        ...
        {
            get { return m_DnsServers[0].Address.ToString(); }
            set
            {
                IPAddress ip;
                ...
                for (int intDnsServer = 0; intDnsServer < m_DnsServers.Count; intDnsServer++)
                {
                    ...
                    try
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (intSoa == 2)
                            {
                                TransferResponse.header.QDCOUNT = TransferResponse.Questions.Count;
                                TransferResponse.header.ANCOUNT = TransferResponse.Answers.Count;
                                TransferResponse.header.NSCOUNT = TransferResponse.Authorities.Count;
                                TransferResponse.header.ARCOUNT = TransferResponse.Additionals.Count;
        ...
        ///</param>
        ///<returns>An System.IAsyncResult instance that references the asynchronous request.</returns>
        public IAsyncResult BeginGetHostAddresses(string hostNameOrAddress, AsyncCallback requestCallback,
                                                  object stateObject)
        {
