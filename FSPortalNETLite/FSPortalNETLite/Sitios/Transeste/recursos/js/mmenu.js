...
function getMouseXY(e) {
    ...
    if (!opera && _d.all) {
        ...
        if (_d.compatMode == "CSS1Compat")MouseY = MouseY + sy;
    };
    return true;
}
...
function sm(v1) {
    ...
    if (opera || ns6 || menu[11])if (!menu[7] && !menu[21][2])fixb(v1);
    ap = gmobj(v1);
    ...
    if (ns4) {
        ...
        if (!opera && !mac && _d.all && menu[9]) {
            ...
            if (navigator.appVersion.indexOf("MSIE 5.5") > 0) {
                menu[9] = menu[9].substr(0, menu[9].indexOf(";"));
            }
            flt = menu[9].split("\;");
