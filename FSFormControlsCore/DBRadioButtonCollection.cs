...
{
    public class DBRadioButtonCollection : CollectionBase, IBindingList
    {
        private ListChangedEventHandler onListChanged;

        public DBRadioButton this[int index]
        {
            get { return (DBRadioButton) List[index]; }
            set { List[index] = value; }
        }

        public DBRadioButton this[string name] => get_Find(name);
        ...
        public DBRadioButton get_Find(string name)
        {
            foreach (DBRadioButton DBRadioButton in List)
                if (DBRadioButton.Name.ToLower() == name.ToLower())
        ...
        public int get_GetColumnOrdinal(string name)
        {
            ...
            foreach (DBRadioButton dbcol in List)
            {
        ...
        public void AddRange(DBRadioButton[] Values)
        {
            var f = 0;
        ...
        public void Remove(DBRadioButton Value)
        {
            List.Remove(Value);
        }


        public void Insert(int index, DBRadioButton Value)
        {
            List.Insert(index, Value);
        }


        public bool Contains(DBRadioButton Value)
        {
            return List.Contains(Value);
        }


        public int IndexOf(DBRadioButton Value)
        {
