...
namespace FSDnsCore
{
    ...
    public class Header
    {
        ...
        public OPCode OPCODE
        {
            ...
            set { Flags = SetBits(Flags, 11, 4, (int) value); }
        }
        ...
        public RCode RCODE
        {
            ...
            set { Flags = SetBits(Flags, 0, 4, (int) value); }
        }

        private int SetBits(int oldValue, int position, int length, bool blnValue)
        {
            return SetBits(oldValue, position, length, blnValue ? 1 : 0);
        }

        private int SetBits(int oldValue, int position, int length, int newValue)
        {
            // sanity check
            ...
            // clear out value
            oldValue &= ~(mask << position);

            // set new value
            oldValue |= ((newValue & mask) << position);
            return oldValue;
        }

        private int GetBits(int oldValue, int position, int length)
        {
            // sanity check
            if (length <= 0 || position >= 16)
                return 0;
            ...
            return ((oldValue >> position) & mask);
        }

        private byte[] WriteShort(int sValue)
        {
